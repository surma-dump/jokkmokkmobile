(function($) {
	var $main = $('#main');
	var $campreview = $('<div><h1></h1><figure><img /></figure></div>');
		...
		$.get(tromso_url)
			.success(function(data) {
				...
				$($.makeArray($imgs).reverse()).each(function(idx, img) {
					...
					var $elem = $campreview.clone().appendTo($main);
					$elem.find('h1').text(format_date(""+img_regexp.exec(src)));
					$elem.find('img').attr('src', tromso_img_url_prefix+src);
				});
			})
			.error(function(msg) {
